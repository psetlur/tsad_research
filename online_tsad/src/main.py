...
if __name__ == "__main__":
    ...
    if schedule_params["n_warmup"] < number_of_random_search:
         logging.warning(f"Warmup iterations ({schedule_params['n_warmup']}) should be >= random search iterations ({number_of_random_search}).")
    # --- End Parameters ---
    ...
    for iter in tqdm(range(total_iterations), desc="Search"):
        ...
        if is_warmup_phase:
            ...
            if iter == schedule_params["n_warmup"] - 1:
                if hasattr(optimizer, '_gp') and optimizer._gp is not None:
                    sigma_avg_current = get_average_uncertainty(optimizer, pbounds, schedule_params["n_uncertainty_samples"])
            ...
            if sigma_avg_initial is None: sigma_avg_history.append(np.nan)
            else: sigma_avg_history.append(sigma_avg_current) # Log calculated value on last step
            ...
            if boost_active_counter > 0:
                ...
                if stagnation_duration >= schedule_params["stagnation_threshold"]:
                    ...
                    if sigma_avg_initial is not None and not np.isnan(sigma_avg_current):
                        ...
                        kappa_to_use_now = np.clip(kappa_adaptive, schedule_params["kappa_min"], schedule_params["kappa_max"])
                    else:
        ...
        if (iter + 1) % 10 == 0:
           tqdm.write(f'  Iter {iter}: WD={loss:.5f}, Kappa={kappa_history[-1]:.4f}, SigmaAvg={sigma_avg_current:.4f}, 1NN_Dist={min_dist:.4f}')
