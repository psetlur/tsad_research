    ...
    def training_step(self, x, batch_idx):
        ...
        for i in range(len(x)):
            ...
            for j in range(num_positives):
                s0 = m0[0] + np.random.uniform(-TAU, TAU)
                s1 = max(m0[1] + np.random.uniform(-TAU, TAU), 0)
                s2 = max(m0[2] + np.random.uniform(-TAU, TAU), 0)
                y_pos[j][i][0] = self.inject_platform(y_pos[j][i][0], s0, s1, s2)
                meta_pos[j].append([s0, s1, s2])

            # generating diverse negative samples
            sampled_negatives = random.sample(negatives, num_negatives)
            for j in range(num_negatives):
                s_neg = sampled_negatives[j]
                y_neg[j][i][0] = self.inject_platform(y_neg[j][i][0], *s_neg)
                meta_neg[j].append(s_neg)

        # concatenating samples
        all_samples = torch.cat([x, y_0] + y_pos + y_neg + [x_pos], dim=0).to(device)
        ...
        c_x = split_outputs[0]
        c_y_0 = split_outputs[1]
        c_y_pos = split_outputs[2:2 + num_positives]
        c_y_neg = split_outputs[2 + num_positives:-1]
        ...
        loss_global = sum(self.info_loss(c_y_0, c_y_p, torch.cat([c_x, c_x_pos], dim=0)) for c_y_p in c_y_pos) / len(c_y_pos)
        
        ### Anomalies with far away hyperparameters should be far away propotional to delta.
        loss_local = sum(hard_negative_loss(c_y_0, c_y_p, torch.stack(c_y_neg), meta_pos[i], meta_neg) 
    ...
    def validation_step(self, x, batch_idx):
        ...
        for i in range(len(x)):
            ...
            for j in range(num_positives):
                ...
                meta_pos[j].append(pos_variation)
            ...
            for j in range(num_negatives):
                ...
                meta_neg[j].append(neg_variation)

        all_samples = torch.cat([x, y_0] + y_pos + y_neg + [x_pos], dim=0).to(device)
        outputs = self(all_samples)

        # Validate output sizes and split outputs
        total_expected_size = x.shape[0] * (2 + num_positives + num_negatives)
        ...
        c_y_0 = split_outputs[1]
        c_y_pos = split_outputs[2:2 + num_positives]
        c_y_neg = split_outputs[2 + num_positives:]
        c_x_pos = split_outputs[-1]
